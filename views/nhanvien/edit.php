...
<section>
  <div class="container">
    <form action="<?php echo vendor_url_util::makeURL(['action' => 'update']) ?>" method="POST" class="form mt-5">
      <input type="hidden" name="old_idnv" value="<?= $this->data['remember']['idnv'] ?>" />
        ...
        <div class="col-6">
          <select class="form-control" name="phongban">
            <?php foreach ($this->data['list_phongban'] as $item) { ?>
              <option value="<?= $item['idpb'] ?>" <?php if ($item['idpb'] == $this->data['remember']['idpb']) echo 'selected'; ?>><?= $item['mota'] ?></option>
            <?php } ?>
          </select>
        </div>
      </div>
